...
export const register = async (userData) => {
  try {
    ...
    // Some backends return 200 even for logical failures (like weak password)
    // We consider it a success ONLY if:
    // 1. success is explicitly true
    // 2. OR status is "success"
    // 3. OR there is no explicit failure indicator AND the message doesn't contain "lemah" (case insensitive)
    ...
    const isSuccess =
      ...
      (data.success !== false &&
        data.status !== "error" &&
        !data.message?.toLowerCase().includes("lemah") &&
        !data.message?.toLowerCase().includes("weak"));

...
const ProductDetail = () => {
            ...
            ? productData.Media.sort((a, b) => a.position - b.position).map(
              (m) =>
                m.media_url.startsWith("http")
                  ? m.media_url
                  : `http://localhost:3000${m.media_url.startsWith("/") ? "" : "/"
                  }${m.media_url}`
            )
  ...
  const handleBuyNow = () => {
    // Clone product logic from handleAddToCart for variant selection
    let variantId = product.id;
    if (product.Variants?.length) {
      const match = product.Variants.find(
        (v) =>
    ...
    const buyNowProduct = {
      ...product,
      variant_id: variantId, // Pass selected variant
      selectedSize,
      selectedColor
    };

    navigate("/buy-now", { state: { product: buyNowProduct } });
  };
  ...
  return (
    <div className="product-detail-page pb-5">
              ...
              <div className="col-2 d-none d-lg-block">
                ...
                >
                  {product.images.map((img, i) => (
                    <div
                      key={i}
                      className={`border rounded cursor-pointer overflow-hidden ${selectedImage === img
              ...
              <div className="col-12 col-lg-10">
                ...
                <div className="d-flex gap-2 mt-2 overflow-x-auto d-lg-none pb-2">
                  {product.images.map((img, i) => (
                    <div
                      key={i}
                      className={`flex-shrink-0 border rounded cursor-pointer ${selectedImage === img
                        ? "border-primary border-2"
                        : "border-light"
                        }`}
          ...
          <div className="col-lg-5">
            <div className="sticky-top" style={{ top: "100px" }}>
              ...
              <div className="d-none d-md-flex gap-2 mb-4">
                <button
                  className="btn btn-dark btn-lg flex-grow-1 d-flex align-items-center justify-content-center"
                ...
                <button
                  className="btn btn-primary btn-lg flex-shrink-0 d-flex align-items-center justify-content-center"
                  onClick={handleAddToCart}
                  style={{ width: "48px", padding: 0, borderRadius: "8px" }}
                  title={t("product.addToCart")}
                >
                  <ShoppingBag size={22} />
                </button>
                <button
                  className="btn btn-outline-dark btn-lg flex-shrink-0 d-flex align-items-center justify-content-center"
                  onClick={() =>
                    addToWishlist(
                      {
                        id: product.id,
      ...
      <div className="fixed-bottom bg-white border-top p-3 d-md-none shadow-lg d-flex gap-2 z-3">
        <button
          className="btn btn-dark flex-grow-1 fw-bold d-flex align-items-center justify-content-center"
          onClick={handleBuyNow}
        ...
        <button
          ...
          onClick={handleAddToCart}
          style={{ width: "45px", borderRadius: "8px", padding: 0 }}
        >
          <ShoppingBag size={20} />
        </button>
        <button
          className="btn btn-outline-dark flex-shrink-0 d-flex align-items-center justify-content-center"
          onClick={() =>
            addToWishlist(
              {
                id: product.id,

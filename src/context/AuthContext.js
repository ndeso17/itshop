...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (userData) => {
    ...
    // Double check: if message contains "lemah", force failure even if result.success is true
    const isActuallySuccess =
      result.success &&
      !result.message?.toLowerCase().includes("lemah") &&
    ...
    if (isActuallySuccess) {
      ...
      // Force RED X icon for failures, especially weak password
      Swal.fire({
        icon: "error",
      ...
      // Ensure we return success: false to the component to prevent redirect
